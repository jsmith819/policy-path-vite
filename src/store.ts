...
const defaultTokenMap: TokenMap = {
  ...
  organisation_short: 'Default Org',   // NEW
  person: 'individual',
...
export const loadTokenMap = (): TokenMap => {
  ...
  const parsed = saved ? (JSON.parse(saved) as Partial<TokenMap>) : {};
  return { ...defaultTokenMap, ...parsed };  // merge so new keys exist
};

export const saveTokenMap = (tm: TokenMap) =>
  localStorage.setItem(TOKEN_MAP_KEY, JSON.stringify(tm));
...
export const saveChangeLog = (log: ChangeLogEntry[]) =>
